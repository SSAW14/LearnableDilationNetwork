...
template<typename Dtype>
void DataTransformer<Dtype>::TransformImgDepthAndSeg(
    const std::vector<cv::Mat>& cv_img_seg,
    Blob<Dtype>* transformed_data_blob,
    Blob<Dtype>* transformed_depth_blob,
    Blob<Dtype>* transformed_label_blob,
    const int ignore_label) {
  CHECK(cv_img_seg.size() == 3) << "Input must contain image, depth and seg.";
  ...
  const int depth_img_channels = cv_img_seg[1].channels();
  // height and width may change due to pad for cropping
  int depth_img_height   = cv_img_seg[1].rows;
  int depth_img_width    = cv_img_seg[1].cols;
  ...
  const int depth_channels = transformed_depth_blob->channels();
  const int depth_height   = transformed_depth_blob->height();
  const int depth_width    = transformed_depth_blob->width();

  const int label_channels = transformed_label_blob->channels();
  const int label_height   = transformed_label_blob->height();
  const int label_width    = transformed_label_blob->width();

//  cout << " asd " << img_channels << " " << img_height << " " << img_width << " " << depth_img_channels << " " << depth_img_height << " " << depth_img_width << " " << seg_channels << " " << seg_height << " " << seg_width << endl;
  CHECK_EQ(seg_channels, 1);
  CHECK_EQ(img_channels, data_channels);
  CHECK_EQ(img_height, seg_height);
  CHECK_EQ(img_width, seg_width);

  CHECK_EQ(depth_img_channels, depth_channels);
  CHECK_EQ(depth_img_height, seg_height);
  CHECK_EQ(depth_img_width, seg_width);
  ...
  CHECK(cv_img_seg[0].depth() == CV_8U)
      << "Image data type must be unsigned byte";
  //CHECK(cv_img_seg[1].depth() == CV_16U)
  //    << "Depth image data type must be unsigned byte";
  ...
  if (scale_factors_.size() > 0) {
    ...
    if (scale != 1) {
      ...
      cv::resize(cv_img_seg[0], cv_cropped_img, cv::Size(img_width, img_height), 0, 0, 
		 cv::INTER_LINEAR);
      cv::resize(cv_img_seg[1], cv_cropped_depth, cv::Size(img_width, img_height), 0, 0, 
		 cv::INTER_LINEAR);
      cv::resize(cv_img_seg[2], cv_cropped_seg, cv::Size(img_width, img_height), 0, 0, 
		 cv::INTER_NEAREST);
    } else {
      cv_cropped_img = cv_img_seg[0];
      cv_cropped_depth = cv_img_seg[1];
      cv_cropped_seg = cv_img_seg[2];
    }
  } else {
    cv_cropped_img = cv_img_seg[0];
    cv_cropped_depth = cv_img_seg[1];
  ...
  // transform to double, since we will pad mean pixel values
  cv_cropped_img.convertTo(cv_cropped_img, CV_64F);
  cv_cropped_depth.convertTo(cv_cropped_depth, CV_64F);
  ...
  if (pad_height > 0 || pad_width > 0) {
    ...
    depth_img_height   = cv_cropped_depth.rows;
    depth_img_width    = cv_cropped_depth.cols;
  ...
  Dtype* transformed_data  = transformed_data_blob->mutable_cpu_data();
  Dtype* transformed_depth  = transformed_depth_blob->mutable_cpu_data();
  Dtype* transformed_label = transformed_label_blob->mutable_cpu_data();

  int top_index;
  const double* data_ptr;
  const double* depth_ptr;
  const uchar* label_ptr;

  //cout << cv_cropped_depth.rows << " " << cv_cropped_depth.cols << " " << cv_cropped_depth.channels() << endl;
  for (int h = 0; h < data_height; ++h) {
    data_ptr = cv_cropped_img.ptr<double>(h);
    depth_ptr = cv_cropped_depth.ptr<double>(h);
    label_ptr = cv_cropped_seg.ptr<uchar>(h);
    ...
    for (int w = 0; w < data_width; ++w) {
      ...
      // for depth
      for (int c = 0; c < depth_img_channels; ++c) {
        if (do_mirror) {
          top_index = (c * depth_height + h) * depth_width + (depth_width - 1 - w);
        } else {
          top_index = (c * depth_height + h) * depth_width + w;
        }
        Dtype pixel = static_cast<Dtype>(depth_ptr[depth_index++]) / 256;
        //cout << pixel << " ";
        if (has_mean_file) {
          int mean_index = (c * depth_img_height + h_off + h) * depth_img_width + w_off + w;
          transformed_depth[top_index] =
            (pixel - mean[mean_index]) * scale;
        } else {
          if (has_mean_values) {
            transformed_depth[top_index] =
            ...
            transformed_depth[top_index] = pixel * scale;
          }
        }
        //cout << c << " " << top_index << " " << pixel << " " << mean_values_[c] << " " << transformed_depth[top_index] << endl;
      }
      //cout << endl;
      // for segmentation
      if (do_mirror) {
